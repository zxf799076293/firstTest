package com.linhuiba.business.activity.searchcity.indexlib.IndexBar.helper;
...
public class IndexBarDataHelperImpl implements IIndexBarDataHelper {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                for (int i1 = 0; i1 < target.length(); i1++) {
                    ...
                    String tagString = Pinyin.toPinyin(target.substring(i1 ,i1+1),"").substring(0, 1);
                    if (tagString.matches("[A-Z]")) {//如果是A-Z字母开头
                        tagSb.append(tagString);
                    } else {//特殊字母这里统一用#处理
                        tagSb.append("#");
                    }
                    pySb.append(Pinyin.toPinyin(target.substring(i1 ,i1+1),"").toUpperCase());
                }
                indexPinyinBean.setBaseIndexPinyin(pySb.toString());//设置城市名全拼音
                indexPinyinBean.setBaseIndexAllTag(tagSb.toString());//设置所有的缩写
            } else {
    ...
    public IIndexBarDataHelper fillInexTag(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                //以下代码设置城市拼音首字母
                if (indexPinyinBean != null &&
                        indexPinyinBean.getBaseIndexPinyin() != null &&
                        indexPinyinBean.getBaseIndexPinyin().length() > 0) {
