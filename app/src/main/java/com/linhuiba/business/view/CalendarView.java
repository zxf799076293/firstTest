...
public class CalendarView extends View implements View.OnTouchListener {
	...
	private List<Integer> dateIndexList = new ArrayList<Integer>();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		surface.height = (int) (getResources().getDisplayMetrics().heightPixels) * 1/2;//日历高度为屏幕的3/4
...
//		}
		...
		for (int i = curStartIndex; i < curEndIndex; i++) {
			...
			if (temp.equals(rightNow.getTime())) {
				color = surface.borderColor;
				drawCellText(canvas, i, date[i] + "", color);
			}
//				if (temp.compareTo(downDate) == 0) {
...
//				* surface.cellHeight + surface.cellHeight * 9 / 7f;
//		float cellX = (surface.cellWidth * (x - 1))
//				+ (surface.cellWidth - surface.datePaint.measureText(text))
//				/ 2f;
		float cellY = surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.cellHeight * 1 / 5f;
		float cellX = (surface.cellWidth * (x - 1))
				+ (surface.cellWidth - surface.datePaint.measureText(text))
				/ 2f;
		float activityY =  surface.monthHeight + surface.weekHeight + (y - 1)
				* surface.cellHeight + surface.cellHeight * 1 / 5f;
		float activityX = (surface.cellWidth * (x - 1));
		Paint fieldPaint = new Paint();
		fieldPaint.setColor(Color.RED);
		fieldPaint.setStyle(Paint.Style.FILL);
		Paint fieldtxtPaint = new Paint();
		fieldtxtPaint.setColor(getResources().getColor(R.color.white));
		fieldtxtPaint.setTextSize(getResources().getDimension(R.dimen.fieldinfo_subsidy_textsize));
		fieldtxtPaint.setAntiAlias(true);

		Paint activityPaint = new Paint();
		activityPaint.setColor(Color.BLUE);
		activityPaint.setStyle(Paint.Style.FILL);
		Paint activitytexPaint = new Paint();
		activitytexPaint.setColor(getResources().getColor(R.color.white));
		activitytexPaint.setTextSize(getResources().getDimension(R.dimen.fieldinfo_subsidy_textsize));
		activitytexPaint.setAntiAlias(true);

		Paint advertisingPaint = new Paint();
		advertisingPaint.setColor(getResources().getColor(R.color.top_title_txt_color));
		advertisingPaint.setStyle(Paint.Style.FILL);
		Paint advertisingtexPaint = new Paint();
		advertisingtexPaint.setColor(getResources().getColor(R.color.white));
		advertisingtexPaint.setTextSize(getResources().getDimension(R.dimen.fieldinfo_subsidy_textsize));
		advertisingtexPaint.setAntiAlias(true);

		canvas.drawCircle(activityX+17, activityY+18, 14, fieldPaint);
		canvas.drawCircle(activityX + surface.cellWidth/2, activityY+18, 14, activityPaint);
		canvas.drawCircle(activityX + surface.cellWidth -17, activityY + 18, 14, advertisingPaint);
		canvas.drawText("1", activityX +14, activityY + 23, fieldtxtPaint);
		canvas.drawText("2",activityX + surface.cellWidth/2 - 3, activityY + 23, activitytexPaint);
		canvas.drawText("3",activityX + surface.cellWidth -22, activityY + 23, advertisingtexPaint);
...
// curDate
		...
		// 0-41
		//for (int i = )
		// in dateIndexList draw selected
		// before now disable
		// unseleted
		// now
		// 选中的颜色显示
...
//		}
		...
		if (y > surface.monthHeight + surface.weekHeight) {
			...
			String s=sdf.format(data);
			try{
				downDate = sdf.parse(s) ;
			}catch(Exception e){

			}
			//

			if (dateList.contains(downDate)) {
				dateList.remove(downDate);
			} else {
				Calendar rightNow = Calendar.getInstance();
				rightNow.setTime(today);
				rightNow.add(Calendar.DAY_OF_YEAR,1);//日期加1天
				Date newdata = rightNow.getTime();
				SimpleDateFormat newsdf=new SimpleDateFormat("yyyy-MM-dd");
				String newstr=sdf.format(newdata);
				try{
					Date NextDate = newsdf.parse(newstr) ;
					if (downDate.after(today) && !(downDate.equals(NextDate))) {
						dateList.add(downDate);
					}
				}catch(Exception e){
			...
			case MotionEvent.ACTION_UP:
				if (downDate != null) {
					if(isSelectMore){
						...
						//响应监听事件
						onItemClickListener.OnItemClick(selectedStartDate,selectedEndDate,downDate);
						onItemClickListener.OnItemsSelcted(dateList);
					}
					invalidate();
				}
	...
	public interface OnItemClickListener {
		...
		void OnItemsSelcted(ArrayList<Date> dateList);
		;
	}
	...
	private class Surface {
		...
		public int todayNumberColor = Color.parseColor("#21b589");
		public int cellDownColor = Color.parseColor("#CCFFFF");
		public int cellSelectedColor = Color.parseColor("#21b589");
		...
		public void init() {
			...
			weekHeight = (float) ((temp + temp * 0.3f) * 0.3);
			...
			float textSize = getResources().getDimension(R.dimen.title_bar_title_txt_size);
...
//			float weekTextSize = weekHeight * 0.6f;
			float weekTextSize = getResources().getDimension(R.dimen.title_bar_title_txt_size);
...
//			float cellTextSize = cellHeight * 0.5f;
			float cellTextSize = getResources().getDimension(R.dimen.title_bar_title_txt_size);
	...
	public static void getdatalist (ArrayList<Date> datelist) {
		 dateList = datelist ;
	}
}
