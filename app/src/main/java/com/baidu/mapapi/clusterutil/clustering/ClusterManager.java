...
//        cluster();
    ...
    public void onMapStatusChangeFinish(MapStatus mapStatus) {
        ...
        if (startLng != null) {
            if (startLng.latitude != finishLng.latitude
                ...
                if (x > 1 || y > 1) {
                    //在这处理滑动
                    android.os.Message message = handler.obtainMessage(result);
                    message.obj = mapStatus;
                    handler.sendMessage(message);
                }
            } else if (startzoom != finishzoom) {
                android.os.Message message = handler.obtainMessage(result);
                message.obj = mapStatus;
                handler.sendMessage(message);
            }
    ...
    public void change_status(MapStatus mapStatus) {
        if (mRenderer instanceof BaiduMap.OnMapStatusChangeListener) {
            ((BaiduMap.OnMapStatusChangeListener) mRenderer).onMapStatusChange(mapStatus);
        }
        // Don't re-compute clusters if the map has just been panned/tilted/rotated. 平移地图时不处理要聚合的点
//        MapStatus position = mMap.getMapStatus();
//        if (mPreviousCameraPosition != null && mPreviousCameraPosition.zoom == position.zoom) {
//            return;
//        }
