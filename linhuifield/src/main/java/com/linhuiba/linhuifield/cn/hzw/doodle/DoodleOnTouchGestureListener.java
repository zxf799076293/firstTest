...
import static com.linhuiba.linhuifield.cn.hzw.doodle.util.DrawUtil.computeAngle;
...
public class DoodleOnTouchGestureListener extends TouchGestureDetector.OnTouchGestureListener {
    ...
    public DoodleOnTouchGestureListener(DoodleView doodle, ISelectionListener listener,Context context) {
        mDoodle = doodle;
            ...
            if (mDoodle.getPen() != DoodlePen.DOT) {
                ...
                if (mDoodle.getPen() == DoodlePen.COPY && mCopyLocation.contains(mDoodle.toX(mTouchX), mDoodle.toY(mTouchY), mDoodle.getSize())) {
                    ...
                    if (mDoodle.getShape() == DoodleShape.HAND_WRITE) { // 手写
                        mCurrDoodlePath = DoodlePath.toPath(mDoodle, mCurrPath,mContext);
                    } else {  // 画图形
                        mCurrDoodlePath = DoodlePath.toShape(mDoodle,
                                mDoodle.toX(mTouchDownX), mDoodle.toY(mTouchDownY), mDoodle.toX(mTouchX), mDoodle.toY(mTouchY),mContext);
                    }
