...
public class DoodlePath extends DoodleRotatableItemBase {
    ...
    public DoodlePath(IDoodle doodle, Context context) {
        super(doodle, 0, 0, 0,context);// 这里默认item旋转角度为0
    }
    ...
    public static DoodlePath toShape(IDoodle doodle, float sx, float sy, float dx, float dy, Context context) {
    ...
    public static DoodlePath toPath(IDoodle doodle, Path p, Context context) {
        DoodlePath path = new DoodlePath(doodle,context);
        path.setPen(doodle.getPen().copy());
        path.setShape(doodle.getShape().copy());
        path.setSize(doodle.getSize());
