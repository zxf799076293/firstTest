package com.linhuiba.linhuifield.CalendarClass;
...
public class LunarCalendar {
	 ...
	 * @return
	 */
	public String getLunarDate(int year_log, int month_log, int day_log, boolean isday,ArrayList<Date> datelist) {
		...
		try {
			...
			NextDate = newsdf.parse(nowadays);
			newstr=newsdf.format(NextDate);
		} catch (ParseException e) {
			e.printStackTrace(); // To change body of catch statement use
			// Options | File Templates.
		}
		...
		if (!isday) {
			...
			for (int i = 0; i < solarHoliday.length; i++) {
				...
				if (sd.trim().equals(smd.trim())) {
                    if (datelist.contains(NextDate)) {
                        return sdv + "月"+ "." + "1"+ "." + newstr;
                    } else {
                        return sdv + "月"+ "." + "0"+ "." + newstr;
                    }
			...
			for (int i = 0; i < lunarHoliday.length; i++) {
				...
				if (ld.trim().equals(lmd.trim())) {
                    if (datelist.contains(NextDate)) {
                        return ldv + "月"+ "." + "1"+ "." + newstr;
                    } else {
                        return ldv + "月"+ "." + "0"+ "." + newstr;
                    }
				}
			}
		}
		if (day == 1) {
			if (datelist.contains(NextDate)) {
				return chineseNumber[month - 1] + "月"+ "." + "1"+ "." + newstr;
			} else {
				return chineseNumber[month - 1] + "月"+ "." + "0"+ "." + newstr;
			}

		} else {
			if (datelist.contains(NextDate)) {
				return getChinaDayString(day) + "月"+ "." + "1"+ "." + newstr;
			} else {
				return getChinaDayString(day) + "月"+ "." + "0"+ "." + newstr;
			}
